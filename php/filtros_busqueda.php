...
<body id="body_busqueda">
    ...
    <div class="contenedor-productos">
        ...
        while ($tenis = mysqli_fetch_assoc($sql)) {
            ?>
           <form action="./insertar_carrito.php" method="POST">
    <article>
        <img src="../img/tenis1.webp">
        <div class="datos-tenis">
            ...
            <!-- Agregar los valores necesarios en los inputs -->
                  <input type="hidden" name="tenis_id" value="<?php echo $tenis['id']; ?>">
                  <input type="hidden" name="cantidad" value="1" min="1" max="<?php echo $tenis['stock']; ?>">

                  <button type="submit" value="insertar_carrito" name="insertar_carrito" id="agregar_carrito">
                    Agregar Carrito <i class="fa-solid fa-cart-shopping"></i>
                  </button>
                </div>
              </article>
            </form>
...
<script>
    function cambiarOrden() {
        ...
        xhr.open('GET', `filtros_busqueda.php?orden=${encodeURIComponent(ordenSeleccionado)}&timestamp=${new Date().getTime()}`, true);
